...
from tools.get_project_info import gpji
from tools.get_apartment_info import gaitool

from livekit.api import LiveKitAPI, ListParticipantsRequest, RoomParticipantIdentity
from onshutdown.nodo import nodo
from utils import outbound_utils
from onshutdown import duccrm
load_dotenv()
...
class DynamicVoiceAgent(Agent):
    ...
    def __init__(self, instructions: str, knowledge_base_tables: list = [], tool_name: list[str] = [], call_info: list[str] = [],ctx:JobContext =[]) -> None:
    ...
    @function_tool(description="A tool for geting information about the project being introduced.")
    async def get_project_info(self, run_ctx: RunContext, question: str) -> str:
        """Geting information about the project being introduced."""
        if not "get_project_info" in self.tool_name:
            return "Get project info tool is not enabled."
        try:
            project_id=self.call_info.get("project_id", "")
            result = await asyncio.to_thread(lambda: asyncio.run(gpji.get_project_info(question,project_id)))
            return result
        except Exception as e:
            logger.error(f"Error in get_project_info: {e}")
            return f"Error in getting project infomation: {str(e)}"
    ...
    async def get_apartment_info(self, run_ctx: RunContext, location: str = "",property_type:str = "", area:str= "",direction:str= "",purpose:str= "",num_rooms:str= "",price:str= "",num_floors:str= "") -> str:
        ...
        try:
            project_id=self.call_info.get("project_id", "")
            result = await asyncio.to_thread(lambda: asyncio.run(gaitool.get_project_info(location,property_type, area,direction,purpose,num_rooms,price,num_floors,project_id)))
            return result
        except Exception as e:
    ...
    async def hang_up_call(self, run_ctx: RunContext) -> str:
        ...
        if not "hang_up_call" in self.tool_name:
            return "hang up call tool is not enabled."
        try:
            ...
            return "Đã dừng cuộc gọi thành công"
        except Exception as e:
...
"""
    ...
    async def _create_agent(self, bot_config: Dict[str, Any],ctx) -> Agent:
    ...
    async def entrypoint(self, ctx: JobContext):
        ...
        try:
            ...
            agent = await self._create_agent(bot_config,ctx)
            ...
            time_start_formatted = outbound_utils.get_current_time()
            if call_info and sip_trunk_id:
                # engress_info,s3_uri_of_recording=await outbound_utils.start_recording(ctx.room.name)
                try:
                    nameroom=ctx.room.name
                    a= await self._remove_non_agent_or_sip(nameroom)
                    ...
                    caller_number=call_info.get("phone_number", "")
                    # Bắt lỗi TwirpError
                    if e.code == livekit_api.TwirpErrorCode.ABORTED and '487' in e.message:
                        #doc_du_lieu(caller_number, "101", "CALL_RING_TIMEOUT","",time_start_call,lead_id)
                        await asyncio.gather(
                            asyncio.to_thread(nodo.doc_du_lieu, caller_number, "101", "CALL_RING_TIMEOUT","",time_start_formatted,lead_id),
                            asyncio.to_thread(nodo.luu_vao_postgres, caller_number, caller_number, "101", "CALL_RING_TIMEOUT","",time_start_formatted,lead_id)
                        )
                        return

                    elif e.code == livekit_api.TwirpErrorCode.UNAVAILABLE and '480' in e.message:
                        #doc_du_lieu(caller_number, "102", "TEMPORARILY_UNAVAILABLE","",time_start_call,lead_id)
                        await asyncio.gather(
                            asyncio.to_thread(nodo.doc_du_lieu, caller_number, "102", "TEMPORARILY_UNAVAILABLE","",time_start_formatted,lead_id),
                            asyncio.to_thread(nodo.luu_vao_postgres, caller_number, "102", "TEMPORARILY_UNAVAILABLE","",time_start_formatted,lead_id)
                        )
                        return 
                    
                    raise e
                except Exception as e:
                    logger.error(f"Đã xảy ra lỗi không mong muốn trong khi gọi SIP: {e}")
                    return 
            ...
            async def on_shutdown():
                await outbound_utils.stop_recording(engress_info)
                caller_number=call_info.get("phone_number", "")
                await outbound_utils.wait_and_process(caller_number,s3_uri_of_recording, duccrm.transcribe_s3_uri)
                # lead_id=call_info.get("lead_id", "")
                # caller_number=call_info.get("phone_number", "")
                # # 2. Phân tích bằng LLM để xác định opportunity
                # await nodo.analyze_and_save_opportunity(ctx.room,self.session,caller_number,time_start_formatted,lead_id)
            # Gắn callback
            if call_info and sip_trunk_id:
