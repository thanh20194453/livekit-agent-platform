...
class DuckDuckGoTools:
    ...
    def duckduckgo_search(self, query: str, max_results: int = 5, region: str = "vn-vi") -> str:
        ...
        ddgs = DDGS(
            proxy=self.proxy, timeout=self.timeout, verify=self.verify_ssl
        )

        return json.dumps(ddgs.text(query=search_query, max_results=actual_max_results, region=region), indent=2)

    def duckduckgo_news(self, query: str, max_results: int = 5, region: Optional[str] = 'vn-vi') -> str:
        ...
        return json.dumps(ddgs.news(query=query, max_results=actual_max_results, region=region), indent=2)
    
    
ddgs = DuckDuckGoTools()